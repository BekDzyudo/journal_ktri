...
function AnnouncementDetail() {
  ...
  useEffect(() => {
    // ID bo'yicha e'lonni topish
    const foundAnnouncement = announcementsData.find(
      (item) => item.id === parseInt(id)
    );
    
    if (foundAnnouncement) {
      setAnnouncement(foundAnnouncement);
      // Ko'rishlar sonini oshirish (API bilan ishlashda backend qiladi)
      // foundAnnouncement.views += 1;
    } else {
      // Agar topilmasa, bosh sahifaga yo'naltirish
  ...
  return (
    <>
      <SEO
        title={`${announcement.title} - KTRI`}
        description={announcement.title}
        keywords={`e'lon, ${announcement.category}, KTRI, ilmiy jurnal`}
      />
              ...
              <div className="relative h-[700px] overflow-hidden">
                ...
                <div className="absolute top-6 left-6">
                  <div className="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full shadow-lg">
                    <FaTags size={14} />
                    <span className="font-semibold text-sm">{announcement.category}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 sm:p-12">
                ...
                <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-6 pb-6 border-b-2 border-gray-100">
                  ...
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span className="text-sm font-medium">{formatDate(announcement.date)}</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600">
                    <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
                      <FaEye size={16} className="text-purple-600" />
                    </div>
                    <span className="text-sm font-medium">{formatViews(announcement.views)} ko'rishlar</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600">
                    <div className="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center">
                      <FaClock size={16} className="text-orange-600" />
                    </div>
                    <span className="text-sm font-medium">{announcement.readTime}</span>
                  </div>
                </div>
                        ...
                        <div className="flex gap-4 p-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-col gap-1 text-xs text-gray-500">
                              ...
                              <span className="flex items-center gap-1">
                                <FaEye size={10} />
                                {formatViews(item.views)} ko'rishlar
                              </span>
                            </div>
                          </div>
                        </div>
